...
class plotting:
    ...
    def hplot(self, data, thresholds, plt_title, leg_title, xlabel, yscale):
        ...
        if thres.size == 1:
            ax.axvline(thres, color=np.array(
                [255, 0, 0])/255, linewidth=1.5, label=thres)
        elif thres.size > 1:

            for ii in range(thres.size):
                ax.axvline(thres[ii], color=np.array(
                    [255, 0, 0])/255, linewidth=1.5, label=thres[ii])
    ...
    def roiview(self, data, thres, plt_type, energy_dispersive_axis='vert'):
        ...
        if plt_type == 'xes':
            ...
            for lim in thres:
                if energy_dispersive_axis == 'horiz' or energy_dispersive_axis == 'horizontal':
                    if thres[lim]:
                        roisum = np.nansum(
                            data[thres[lim][0]:thres[lim][1], :], axis=1)
                        p2 = ax[1].plot(roisum, linewidth=1.5, label=lim)
                        for ii in range(len(thres[lim])):
                            ax[0].axhline(thres[lim][ii], color='red', linewidth=1.5,
                                          label=lim + ': {}'.format(thres[lim][ii]))
                else:
                    if thres[lim]:
                        roisum = np.nansum(
                            data[:, thres[lim][0]:thres[lim][1]], axis=1)
                        p2 = ax[1].plot(roisum, linewidth=1.5, label=lim)
                        for ii in range(len(thres[lim])):
                            ax[0].axvline(thres[lim][ii], color='red', linewidth=1.5,
                                          label=lim + ': {}'.format(thres[lim][ii]))
            ...
            for lim in thres:
                if thres[lim] and lim == 'horiz':
                    for ii in range(len(thres[lim])):
                        ax.axvline(thres[lim][ii], color='red', linewidth=1.5,
                                   label=lim + ': {}'.format(thres[lim][ii]))
                elif thres[lim] and lim == 'vert':
                    for ii in range(len(thres[lim])):
                        ax.axhline(thres[lim][ii], color='red', linewidth=1.5,
                                   label=lim + ': {}'.format(thres[lim][ii]))
        ...
        # cb.ax.get_children()[7] is colorbar spine
        cb.ax.get_children()[7].set_linewidth(width)
        fig.tight_layout()
        plt.show()
...
class diagnostics(plotting):
    def __init__(self, run, exp, keys, friendly_names):
        ...
        fpath = '/sdf/data/lcls/ds/{}/{}/hdf5/smalldata/'.format(
            self.exp[:3], self.exp)
    ...
    def adu_histogram(self, nshots, thresholds, ROIopt=False, energy_dispersive_axis='vert'):
        ...
        pt = 'Pixel Intensity on ePix100 Run {} ({} shots)'.format(
            self.run, nshots)
        lt = 'ADU Thresholds'
        ...
        if ROIopt:
            if hasattr(self, 'xes_roi_limits'):
                ...
                for lim in thres:
                    if thres[lim]:
                        if energy_dispersive_axis == 'horiz' or energy_dispersive_axis == 'horizontal':
                            data2plot = self.h5[self.datadict['epix']][0:nshots,
                                                                       thres[lim][0]:thres[lim][1], :].ravel()
                        else:
                            data2plot = self.h5[self.datadict['epix']][0:nshots,
                                                                       :, thres[lim][0]:thres[lim][1]].ravel()

                        pt = 'Pixel Intensity in {} ROI ({} shots)'.format(
                            lim, nshots)
                ...
                data2plot = self.h5[self.datadict['epix']][0:nshots, thres['vert']
                                                           [0]:thres['vert'][1], thres['horiz'][0]:thres['horiz'][1]].ravel()
                pt = 'Pixel Intensity in XAS ROI ({} shots)'.format(nshots)
                self.hplot(data2plot, thresholds, pt, lt, xl, ys)

            else:
                print(
                    'Error: no ROIs have been set, run function xes_ROI or xas_ROI with option setrois = True)')
                return
    ...
    def xes_ROI(self, nshots, kb_limits=[], ka_limits=[], setrois=False, energy_dispersive_axis='vert', angle=0):
        ...
        data2plot = np.nansum(
            self.h5[self.datadict['epix']][0:nshots, :, :], axis=0)
        data2plot_rot = rotate(data2plot, angle, axes=[0, 1])

        ptype = 'xes'

        self.roiview(data2plot, roi_limits, ptype,
                     energy_dispersive_axis=energy_dispersive_axis)
