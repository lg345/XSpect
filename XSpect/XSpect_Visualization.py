...
class SpectroscopyVisualization:
    ...
    def plot_2d_spectrum(self, run, detector_key):
        ...
        im = ax.imshow(spectrum, vmin=vmin, vmax=vmax,
...
class XESVisualization(SpectroscopyVisualization):
    ...
    def plot_2d_difference_spectrum(self, xes_analysis):
        ...
        plt.figure(dpi=100)
        plt.imshow(difference_spectrum.T, cmap='RdBu', vmin=self.vmin, vmax=self.vmax, origin='lower',
                   aspect='auto', extent=[xes_analysis.mintime, xes_analysis.maxtime, energy[0], energy[-1]])
        plt.colorbar()
        plt.xlabel('Time (ps)')
...
class XASVisualization(SpectroscopyVisualization):
    ...
    def combine_spectra(self, xas_analysis, xas_laser_key, xas_key, norm_laser_key, norm_key, interpolate=False):
        ...
        for idx, run in enumerate(xas_analysis.analyzed_runs):
            if idx > 0 and interpolate:
                ...
                # assuming time axis is the first dimension
                time = np.arange(xas_laser.shape[0])

                interp_laser_on = interp2d(ccm, time, getattr(
                    run, xas_laser_key), fill_value=0, bounds_error=False)
                interp_laser_off = interp2d(ccm, time, getattr(
                    run, xas_key), fill_value=0, bounds_error=False)
                interp_norm_on = interp2d(ccm, time, getattr(
                    run, norm_laser_key), fill_value=0, bounds_error=False)
                interp_norm_off = interp2d(ccm, time, getattr(
                    run, norm_key), fill_value=0, bounds_error=False)
    ...
    def plot_2d_difference_spectrum(self, xas_analysis, vmin=None, vmax=None):
        ...
        laser_off_spectrum = np.divide(np.nansum(
            xas_analysis.summed_laser_off, axis=0), np.nansum(xas_analysis.summed_norm_off, axis=0))
        difference_spectrum = laser_on_spectrum-laser_off_spectrum
...
#         plt.imshow(difference_spectrum.T, cmap='RdBu', vmin=self.vmin, vmax=self.vmax, origin='lower',aspect='auto',extent=[xas_analysis.mintime,xas_analysis.maxtime,xas_analysis.minccm,xas_analysis.maxccm])
        ...
        plt.contourf(xas_analysis.ccm_bins, xas_analysis.time_bins,
                     difference_spectrum, contlevels, cmap='RdBu', extend="max")
        plt.colorbar()
